    ...
    for j = 1:N_c  
        ...
        for p = 1:count
            ...
            for q = 1:count
                ...
                index=(4+4*N_p)^2*(N_c)*(i-1)+(4+4*N_p)^2*(j-1)+(4+4*N_p)*(p-1)+q;
                I(index)=global_p;
                J(index)=global_q;
                K(index)=k(p, q);
            end
...
b=zeros(1,4+4*N_c*N_p+2*(N_c-4)*(N_c-1));
b(1:4+2*N_c*N_p)=[1,N_c+1,(N_c+1)*N_c+1,(N_c+1)*(N_c+1),...
    (N_c+1)^2+1:(N_c+1)^2+N_p*N_c,...
    (N_c+1)^2+1+N_c^2*N_p:(N_c+1)^2+N_p*N_c+N_c^2*N_p];
for i=1:N_p
    b(4+2*N_c*N_p+1+(i-1)*2*N_c:4+2*N_c*N_p+(i)*2*N_c)=...
...
for i=1:N_c-1
    b(4+4*N_c*N_p+1+(i-1)*(N_c-4):4+4*N_c*N_p+(i)*(N_c-4))=...
        [(N_c+1)^2+3*N_p+N_p*i*N_c:N_p:(N_c+1)^2+(N_c-2)*N_p+N_p*i*N_c];
end
for i=1:N_c-1
    b(4+4*N_c*N_p+1+(N_c-4)*(N_c-1)+(i-1)*(N_c-4):4+4*N_c*N_p+(N_c-4)*(N_c-1)+(i)*(N_c-4))=...
        [i*N_p+N_p+N_c*(N_c+1)*N_p+(N_c+1)^2+2*N_p*(N_c+1):N_p*(N_c+1):i*N_p+N_p+N_p*(N_c+1)*(N_c-3)+N_c*(N_c+1)*N_p+(N_c+1)^2];
end
