...
if n>1
    ...
    [R,P,bub] = restrict(X, Y, m, n-1,N_f,1,k0);
...
if n<N_c
    ...
    [R,P,bub] = restrict(X, Y, m, n,N_f,1,k0);
    [V,D]=eigs(R'*N*R,P,N_e);
...
if m>1
    ...
    [R,P,bub] = restrict(X, Y, m-1, n,N_f,2,k0);
    [V,D]=eigs(R'*N*R,P,N_e);
    value(:,count+1:count+N_e)=L2*R*V;
    value(:,count+N_e+1)=L2*bub;
else
...
if m<N_c
    ...
    [R,P,bub] = restrict(X, Y, m, n,N_f,2,k0);
    [V,D]=eigs(R'*N*R,P,N_e);
    value(:,count+1:count+N_e)=L1*R*V;
    value(:,count+N_e+1)=L1*bub;
else
...
for u = 1:N_f
    ...
    for v = 1:N_f
        ...
        for x1=0:1
            for y1=0:1
                F(v*N_f+u-N_f,:) = F(v*N_f+u-N_f,:)+ffun(xmean,ymean)*value((v+y1-1)*(N_f+1)+u+x1,:)*(x(2)-x(1))^2/4;
            end
        end
    end
end
for u = 1:N_f
    ...
    ymean=(y(u+1)+y(u))/2;
    for x1=0:1
        F(N_f*N_f+u,:) = F(N_f*N_f+u,:)+gfun(xmean,yhigh,k0)*value(N_f*(N_f+1)+u+x1,:)*(x(2)-x(1))/2;
        F(N_f*N_f+u+N_f,:) = F(N_f*N_f+u+N_f,:)+gfun(xmean,ylow,k0)*value(u+x1,:)*(x(2)-x(1))/2;
        F(N_f*N_f+u+2*N_f,:) = F(N_f*N_f+u+2*N_f,:)+gfun(xhigh,ymean,k0)*value((u+x1-1)*(N_f+1)+N_f+1,:)*(x(2)-x(1))/2;
        F(N_f*N_f+u+3*N_f,:) = F(N_f*N_f+u+3*N_f,:)+gfun(xlow,ymean,k0)*value((u+x1-1)*(N_f+1)+1,:)*(x(2)-x(1))/2;
    end
end
f=sum(F);
end
