...
y = x;
N_p=N_e+1;
nNodes = (N_c+1)*(N_c+1)+2*N_c*(N_c+1)*N_p;
...
b=zeros(1,4*(N_c)+4*N_c*N_p);
b(1:4*(N_c)+2*N_c*N_p)=[1:N_c+1,N_c+2:N_c+1:(N_c+1)*(N_c+1),2*(N_c+1):N_c+1:(N_c+1)*(N_c+1),...
    N_c*N_c+N_c+2:(N_c+1)*(N_c+1)-1,(N_c+1)^2+1:(N_c+1)^2+N_p*N_c,...
    (N_c+1)^2+1+N_c^2*N_p:(N_c+1)^2+N_p*N_c+N_c^2*N_p];
for i=1:N_p
    b(4*N_c+2*N_c*N_p+1+(i-1)*2*N_c:4*N_c+2*N_c*N_p+(i)*2*N_c)=...
